...
# 32_bootstrap_ci.R (all replicates of the four axis dirs pooled by sigma_g2;
...
# Usage:  Rscript sim/bench_full/60_make_exact_tables.R
# ==============================================================================
...
supp_axis <- function(ax, var, var_label, levels_fmt) {
  # side-by-side regimes: one row per method and level, sigma_g2 = 0 | 0.5
  D <- read_axis(EX, ax)
  D <- D[D$method %in% ROSTER, ]
  lv <- sort(unique(D[[var]]))
  cell <- function(mm, l, sg) {
  ...
  for (l in lv) {
    body <- c(body,
      ...
      sprintf("\\multicolumn{9}{l}{\\textbf{%s}} \\\\", sprintf(levels_fmt, l)),
      vapply(ROSTER, function(mm)
        sprintf("%s & %s & %s \\\\", DISPLAY[mm], cell(mm, l, 0), cell(mm, l, 0.5)),
        character(1)))
  }
  c("\\begin{tabular}{lrrrrrrrr}", "\\toprule",
    "& \\multicolumn{4}{c}{$\\sigma_g^2=0$} & \\multicolumn{4}{c}{$\\sigma_g^2=0.5$} \\\\",
    "\\cmidrule(lr){2-5}\\cmidrule(lr){6-9}",
    paste("Method & $\\overline{F_1}$ & Rec. & Prec. & $\\overline{\\hat K}$",
...
if ("signal" %in% names(arch_raw)) {
  ...
  body <- character(0)
  for (l in c("weak", "medium", "strong")) {
    body <- c(body,
      "\\midrule"[length(body) > 0],
      sprintf("\\multicolumn{9}{l}{\\textbf{%s signal}} \\\\", l),
      vapply(ROSTER, function(mm)
        sprintf("%s & %s & %s \\\\", DISPLAY[mm], cell(mm, l, 0), cell(mm, l, 0.5)),
        character(1)))
  }
