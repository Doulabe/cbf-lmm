# ==============================================================================
#  23_amortized_threshold_grid.R
#  Extends the threshold-independent comparison of 20_amortized_topk.R
#  from the original 6-cell focused subset to the full 18-cell axis grid.
#  We reuse the per-SNP score extractors from 20_amortized_topk.R
...
#    Rscript sim/bench_full/23_amortized_threshold_grid.R --cores 8 --B 100
# ==============================================================================
...
# 20_amortized_topk.R (defines topK_*, score_pip_*, compute_metrics).
# We invoke it with sys.source so its main {if (sys.nframe()==0L)} block
# does not auto-run.
local({
  e <- new.env(parent = .GlobalEnv)
  sys.source("sim/bench_full/20_amortized_topk.R", envir = e,
             toplevel.env = e)
...
if (!exists("run_cell_threshold_indep", envir = .GlobalEnv, inherits = FALSE)) {
  stop("run_cell_threshold_indep() not exported from 20_amortized_topk.R\n",
...
sink(tex_path)
cat("% Auto-generated by sim/bench_full/23_amortized_threshold_grid.R\n")
